  ...
  def run(ctx: Context)(p: N.Program): (S.Program, SymbolTable) = {
    ...
    def transformType(tt: N.TypeTree, inModule: String): S.Type = {
      tt.tpe match {
        ...
        case N.TupleType(types) => S.TupleType(types.map(t => S.TypeTree(transformType(t, inModule))))
    ...
    def transformExpr(expr: N.Expr)
      ...
      val res = expr match {
        ...
        case N.Match(scrut, cases) =>
          ...
          def transformPattern(pat: N.Pattern): (S.Pattern, List[(String, Identifier)]) = {
            val (newPat, newNames): (S.Pattern, List[(String, Identifier)]) = pat match {
              ...
              case N.TuplePattern(pats) => 
                val (pat, newBinding)  = pats.map(transformPattern(_)).unzip
                (S.TuplePattern(pat), newBinding.flatten)  
        ...
        case N.TupleExpr(exprs) => S.TupleExpr(exprs.map(transformExpr(_)))
        case N.Error(msg) =>
